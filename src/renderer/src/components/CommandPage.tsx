...
export const CommandPage = ({
  ...
  return (
    <Command loop>
      <div className="flex items-center glass-divider px-5 py-4">
        <Search className="mr-2.5 h-4 w-4 shrink-0 text-white/35" />

        <input
          autoFocus
          className="flex h-11 w-full rounded-xl bg-transparent py-3 text-sm outline-hidden placeholder:text-white/35 disabled:cursor-not-allowed disabled:opacity-50"
          value={commandParam}
        ...
        <CommandEmpty className="flex flex-col h-80 items-center justify-center">
          {isLoadingResult && (
            <div className="flex flex-col items-center">
              <i className="ph ph-hourglass-high text-8xl text-white/15 animate-spin" />
              <p className="text-2xl text-white/35 italic">Waiting for result...</p>
            </div>
          )}

          {noResult && (
            <div className="flex flex-col items-center">
              <i className="ph ph-empty text-8xl text-white/15" />
              <p className="text-2xl text-white/35 italic">There is no result.</p>
            </div>
          )}

          {emptyResult && (
            <div className="flex flex-col items-center">
              <i className="ph ph-textbox text-8xl text-white/15" />
              <p className="text-2xl text-white/35 italic">Type a command parameter.</p>
            </div>
          )}
        </CommandEmpty>
      ...
      <Footer>
        <div className="flex flex-row items-center">
          ...
          {showSubCommand && (
            <>
              <hr className="ml-4 mr-4 border-0 border-l border-l-white/8 h-3" />
              <SubCommand
                actions={actions}
